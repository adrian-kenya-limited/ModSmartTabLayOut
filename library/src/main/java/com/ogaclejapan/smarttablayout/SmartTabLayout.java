...
public class SmartTabLayout extends HorizontalScrollView {
    ...
    private int textViewTypefaceResId;
    private boolean tabViewTextAllCaps;
    private boolean tabViewTextAllBold;
    ...
    Context classWideContext;

    public SmartTabLayout(Context context) {
        this(context, null);
        this.classWideContext = context;
    }
    ...
    public SmartTabLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        int textVwTypefaceResId = 0;
        boolean textAllCaps = TAB_VIEW_TEXT_ALL_CAPS;
        boolean textAllBold = true;
        ...
        textVwTypefaceResId = a.getResourceId(
                R.styleable.stl_SmartTabLayout_stl_defaultTextViewFont, textVwTypefaceResId);
        textAllCaps = a.getBoolean(
                R.styleable.stl_SmartTabLayout_stl_defaultTabTextAllCaps, textAllCaps);
        textAllBold = a.getBoolean(
                R.styleable.stl_SmartTabLayout_stl_defaultTabTextAllBold, textAllBold);
        ...
        this.textViewTypefaceResId = textVwTypefaceResId;
        this.tabViewTextAllCaps = textAllCaps;
        this.tabViewTextAllBold = textAllBold;
    ...
    protected TextView createDefaultTabView(CharSequence title) {
        ...
        if(tabViewTextAllBold){
            textView.setTypeface(getTypeface(textViewTypefaceResId),Typeface.BOLD);
        }else {
            textView.setTypeface(getTypeface(textViewTypefaceResId),Typeface.NORMAL);
        }
        //textView.setText(ifBoldText(tabViewTextAllBold, title.toString()));
        //textView.setText(ifBoldText(tabViewTextAllBold, (title != null ? title.toString() : null)));
    ...
    public Typeface getTypeface(int fontResId) {
        Typeface typefaceDefault = null;
        try {
            if (fontResId != 0) {
                typefaceDefault = ResourcesCompat.getFont(classWideContext, fontResId);
            }
        } catch (Exception xx) {
            xx.printStackTrace();
        }
    ...
    public String ifBoldText(boolean textAllBold, String strText) {
        String rtnStr = strText;
        if (strText != null) {
            if (textAllBold) {
                rtnStr = Html.fromHtml("<b>" + strText + "</b>").toString();
            }
        }
        return rtnStr;
    }
